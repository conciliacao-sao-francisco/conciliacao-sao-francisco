...
st.markdown("""
    ...
    /* Ajuste para alinhar verticalmente o popover de edição com o checkbox */
    div[data-testid="stPopover"] { margin-top: 4px; }
    </style>
""", unsafe_allow_html=True)
        ...
        for linha in texto.split('\n'):
            ...
            if match_data and "SALDO" not in linha_u:
                ...
                if len(partes) >= 3:
                    ...
                    if v_num != 0.0:
                        desc = inline_u.replace(dt_enc, "").replace(partes[-1], "").strip()
...
# =========================================================================
    ...
    chave_edicoes = f"edicoes_sistema_{conta_ativa}"  # Guarda as edições manuais feitas no Boletim
    ...
    if chave_edicoes not in st.session_state: st.session_state[chave_edicoes] = {}
    ...
    # Aplica na memória as edições feitas pelo usuário nos dados do Sistema antes de renderizar
    if df_sistema_orig is not None and not df_sistema_orig.empty:
        for t_id, alteracoes in st.session_state[chave_edicoes].items():
            idx_match = df_sistema_orig.index[df_sistema_orig['id_theos'] == t_id]
            if not idx_match.empty:
                df_sistema_orig.loc[idx_match, 'Valor'] = alteracoes['Valor']
                df_sistema_orig.loc[idx_match, 'Descrição'] = alteracoes['Descrição']
        ...
        with aba_conciliacao:
            ...
            with col2:
                ...
                for _, row in df_sistema_tela.iterrows():
                    ...
                    # Layout dividido: Checkbox do item ocupará a maior parte e um popover compacto ao lado para edição rápida
                    ...
                    with col_edit:
                        with st.popover("⚙️", help="Ajustar dados deste registro do sistema"):
                            ...
                            novo_desc = st.text_input("Alterar Descrição:", value=row['Descrição'], key=f"desc_inp_{row['id_theos']}")
                            novo_val = st.number_input("Alterar Valor (R$):", value=float(row['Valor']), step=1.00, format="%.2f", key=f"val_inp_{row['id_theos']}")
                            if st.button("Salvar Correção", key=f"btn_salvar_{row['id_theos']}", use_container_width=True):
                                st.session_state[chave_edicoes][row['id_theos']] = {'Descrição': novo_desc, 'Valor': novo_val}
                                st.rerun()
