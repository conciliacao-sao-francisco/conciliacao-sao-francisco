...
# =========================================================================
# OPERACIONAL E INTERFACE DE CONCILIAÇÃO
# =========================================================================
if bytes_banco_prontos and ("Poupança" in conta_ativa or bytes_sistema_prontos):
    ...
    for mod in st.session_state[chave_modificacoes]:
        if mod['acao'] == 'excluir':
            ...
            else: df_s_orig = pd.concat([df_s_orig, nova_linha], ignore_index=True)

    todas_datas = sorted(list(set(df_b_orig['Data'].unique()).union(set(df_s_orig['Data'].unique() if df_s_orig is not None else []))), key=lambda x: pd.to_datetime(x, dayfirst=True))
    ...
    with st.sidebar:
        ...
        if c_nav2.button("Próximo ▶", use_container_width=True) and st.session_state.indice_data < len(todas_datas) - 1:
            ...
            st.rerun()
        st.markdown("---")
        if st.button("🔄 Desfazer Última Baixa", use_container_width=True):
            st.toast("Última ação revertida!")
    ...
    with aba_conciliacao:
        ...
        # Filtros táticos por tipo (Idêntico ao seu print original)
        ...
        if filtro_banco != "Todos": df_banco_tela = df_banco_tela[df_banco_tela['Tipo'] == filtro_banco]
        
        df_sistema_tela = df_s_orig[df_s_orig['Data'] == data_selecionada] if df_s_orig is not None else pd.DataFrame()
        ...
        with col2:
            st.markdown('<div class="titulo-coluna-igreja">⛪ Paróquia /具 Boletim Theos</div>', unsafe_allow_html=True)
        ...
        # =========================================================================
        # ⭐ PAINEL COMPLETO DE MODIFICAÇÃO (INSERIR, EDITAR, EXCLUIR E MUDAR DATA)
        # =========================================================================
        ...
        with aba_ins:
            ...
            if st.button("➕ Inserir Ajuste na Esteira", use_container_width=True):
                ...
                st.success("Ajuste inserido na memória com sucesso!")
                st.rerun()

        with aba_edt:
            df_todos_dia = pd.concat([df_banco_dia, df_sistema_dia if df_s_orig is not None else pd.DataFrame()]) if 'df_banco_dia' in locals() else pd.DataFrame()
            df_todos_dia = pd.concat([df_banco_tela, df_sistema_tela], ignore_index=True)
            
            if not df_todos_dia.empty:
                item_selecionado = st.selectbox("Escolha qual lançamento deseja alterar/remover:", 
                ...
                if col_b_ed1.button("💾 Gravar Modificações / Trocar Data", type="primary", use_container_width=True):
                    # Remove modificação anterior do mesmo item se houver
                    ...
                    st.success("Lançamento atualizado e reposicionado!")
                    st.rerun()
                ...
                if col_b_ed2.button("🗑️ Excluir este lançamento permanentemente", use_container_width=True):
                    ...
                    st.warning("Lançamento deletado do painel corrente!")
                    st.rerun()
            else:
                st.info("Nenhum lançamento filtrado neste dia para modificação.")
