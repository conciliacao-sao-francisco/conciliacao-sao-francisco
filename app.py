        ...
        for linha in texto.split('\n'):
            ...
            if match_data and "SALDO" not in linha_u:
                ...
                if len(partes) >= 3:
                    ...
                    if v_num != 0.0:
                        desc = inline_u = linha_u.replace(dt_enc, "").replace(partes[-1], "").strip()
...
# =========================================================================
    ...
    if df_banco_orig is not None and not df_banco_orig.empty:
        ...
        with aba_conciliacao:
            ...
            with col1:
                st.markdown('<div class="titulo-coluna"><span style="font-size:24px;">🏦</span><div class="texto-header-col">Extrato Sicoob<br><span style="font-size:12px; color:#666; font-weight:normal;">' + str(len(df_banco_tela)) + ' pendentes para fechar</span></div></div>', unsafe_allow_html=True)
            ...
            with col2:
                ...
                st.markdown('<div class="titulo-coluna-igreja"><span style="font-size:24px;">⛪</span><div class="texto-header-col">' + lbl_sist + '<br><span style="font-size:12px; color:#666; font-weight:normal;">' + str(len(df_sistema_tela)) + ' pendentes no sistema</span></div></div>', unsafe_allow_html=True)
